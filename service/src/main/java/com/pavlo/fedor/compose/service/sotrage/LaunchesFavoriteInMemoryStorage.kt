...
class LaunchesFavoriteInMemoryStorage(private val launchesPageStorage: LaunchesPageStorage) : FavoriteLaunchesPageStorage {
    ...
    override suspend fun replace(value: LaunchInfo): Flow<Unit> = flowOf(stateFlow.value)
        ...
        .flatMapConcat { launchesPageStorage.replace(value) }
        .flowOn(Dispatchers.IO)
